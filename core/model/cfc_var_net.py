...
class CfCNet_VAR(nn.Module):
    def __init__(self,config):
        super(CfCNet_VAR,self).__init__()
        ...
        fcn_var = nn.ModuleList()
        fcn_var.append(nn.Linear(motor_size,width_fcn))
        for i in range(0,layers_fcn-1):
            if i != layers_fcn - 2:
                layer_in = width_fcn//(decay_fcn**i)
                layer_out = width_fcn//(decay_fcn**(i+1))
                fcn_var.append(nn.Linear(layer_in,layer_out))
                if config['norm']:
                    fcn_var.append(nn.LayerNorm(layer_out,eps = 1e-6))
                fcn_var.append(activation)
            else:
                layer_in = width_fcn//(decay_fcn**i)
                fcn_var.append(nn.Linear(layer_in,output_size))
        self.fcn_var = nn.Sequential(*fcn_var)
        self.init_weight_nn(self.fcn_var)
    ...
    def forward(self,x,t,state):
        ...
        mu = self.fcn(output[:,-1])
        var = self.fcn_var(output[:,-1])
        var.exp_()
        return mu,var,(new_h,new_c)
    ...
    def fit(self,loader_train,loader_test,params):
        ...
        for epoch in epoch_iterator:
            ...
            for x_batch,y_batch,t_batch in minibatch_iterator:
                optimizer.zero_grad()
                h,c = h.data,c.data
                mu,var,_ = self(x_batch.to(device = self.device,dtype = self.dtype),t_batch.to(device = self.device,dtype = self.dtype),(h,c))
                loss = nll(mu,y_batch.to(device = self.device,dtype = self.dtype),var)
                loss.backward()
    ...
    def predict(self,loader,config):
        ...
        with torch.no_grad():
            ...
            for x,y,t in minibatch_iterator:
                ...
                _,_,state = self(x.to(device = self.device,dtype = self.dtype),t.to(device = self.device,dtype = self.dtype),state)  
                mu,var,_ = self(x.to(device = self.device,dtype = self.dtype),t.to(device = self.device,dtype = self.dtype),state)  
                mu_predicted.append(mu)
                var_predicted.append(var)
                y_real.append(y.to(device = self.device,dtype = self.dtype))
                x_used.append(x[:,-1].to(device = self.device,dtype = self.dtype))
            return torch.cat(mu_predicted,dim = 0),torch.cat(var_predicted,dim = 0),torch.cat(y_real,dim = 0),torch.cat(x_used,dim = 0)
    ...
    def predict2(self,loader,batch_size):
        ...
        with torch.no_grad():
            ...
            for x,t in minibatch_iterator:
                ...
                var_predicted.append(var)
                x_used.append(x[:,-1].to(device = self.device,dtype = self.dtype))
            return torch.cat(mu_predicted,dim = 0),torch.cat(var_predicted,dim = 0),torch.cat(x_used,dim = 0)
    ...
    def predict_t(self,loader,config):
        ...
        with torch.no_grad():
            ...
            for x,y,t in minibatch_iterator:
                state = self.init_hidden(batch_size)
                _,_,state = self(x.to(device = self.device,dtype = self.dtype),t.to(device = self.device,dtype = self.dtype),state)  
                mu,var,state = self(x.to(device = self.device,dtype = self.dtype),t.to(device = self.device,dtype = self.dtype),state)  
                mu_predicted.append(mu)
                var_predicted.append(var)
                y_used.append(y.to(device = self.device,dtype = self.dtype))
            nll = nn.GaussianNLLLoss()
            mu_pred = torch.cat(mu_predicted,dim = 0)
            var_pred = torch.cat(var_predicted,dim = 0)
            y_real = torch.cat(y_used,dim = 0)
            error  = nll(mu_pred,y_real,var_pred).item()
            return error