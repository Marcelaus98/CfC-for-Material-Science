...
class CfCCell(nn.Module):
    def __init__(
        ...
        if mode == 'pure'and sparsity_mask is not None: 
            self.sparsity_mask = torch.nn.Parameter(
                data=torch.from_numpy(np.abs(sparsity_mask.T).astype(np.float32)),
                requires_grad=False,
            )
        elif mode != 'pure' and sparsity_mask is not None:
            sparsity_mask_temp = torch.from_numpy(np.abs(sparsity_mask.T).astype(np.float32))
            sparsity_mask_temp = torch.cat((sparsity_mask_temp.tile(2,1),torch.ones(2*hidden_size,cat_shape)),dim = 0)
            self.sparsity_mask = torch.nn.Parameter(data = sparsity_mask_temp,requires_grad = False)
        ...
        if self.mode == "pure":
            ...
            self.gates_cells = nn.Linear(cat_shape, 4*hidden_size)
            #self.ff1 = nn.Linear(cat_shape, hidden_size)
    ...
    def forward(self, input, hx, ts):
        ...
        if self.mode == "pure":
            ...
            # Cfc
            if self.sparsity_mask is not None:
                gates = F.linear(x, self.gates_cells.weight * self.sparsity_mask, self.gates_cells.bias)
            else:
                gates = self.gates_cells(x)
            ff1,ff2,t_a,t_b = gates.chunk(4,1)
            ff1 = self.tanh(ff1)
            ff2 = self.tanh(ff2)
